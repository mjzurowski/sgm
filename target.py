...
#The overall excpected units including the coefficients, i.e. cp cp Fij should be 
    ...
    def F33(self,ER,cp,cn,vm):
        ...
        h = np.power(self.Q(ER),2.)*(h_p_p+h_p_n+h_n_p+h_n_n)/8  #units=ev^2
        ...
        g = (g_p_p+g_p_n+g_n_p+g_n_n)/4  #units=ev^2
 ...
 #jx? fix this   
    def F77(self,ER,cp,cn,vm):  #no jx
        """
  ...
  #jx? check this  
    ...
    def F1111(self,ER,cp,cn,jx): #jx
        ...
        n_p = cn*cp*self.FMnp(ER)
        n_n = cn*cn*self.FMnn(ER)
        return self.spin_dep(jx)*np.power(self.Q(ER),2.)*(p_p+p_n+n_p+n_n)/4  #units=ev^2

    
    def F1212(self,ER,cp,cn,vm,jx): #jx vm
        """
        ...
        g_p_p = cp*cp*(np.power(self.Q(ER)/mp,2.)*(self.FPhi2pp(ER) + self.FPhipp(ER))-np.power(vm,2.)*(self.FS1pp(ER)/2 + self.FS2pp(ER)))
        g_p_n = cp*cn*(np.power(self.Q(ER)/mp,2.)*(self.FPhi2pn(ER) + self.FPhipn(ER))-np.power(vm,2.)*(self.FS1pn(ER)/2 + self.FS2pn(ER)))
        g_n_p = cn*cp*(np.power(self.Q(ER)/mp,2.)*(self.FPhi2np(ER) + self.FPhinp(ER))-np.power(vm,2.)*(self.FS1np(ER)/2 + self.FS2np(ER)))
        g_n_n = cn*cn*(np.power(self.Q(ER)/mp,2.)*(self.FPhi2nn(ER) + self.FPhinn(ER))-np.power(vm,2.)*(self.FS1nn(ER)/2 + self.FS2nn(ER)))
        g = self.spin_dep(jx)*(g_p_p+g_p_n+g_n_p+g_n_n)/16  #units=untiless

        return [g,h]
   
    
    def F1313(self,ER,cp,cn): #jx vm
        """
        ...
        g_p_p = cp*cp*(np.power(self.Q(ER)/math.sqrt(mp),4.)*self.FPhipp(ER)-np.power(vm*self.Q(ER),2.)*self.FS2pp(ER))
        g_p_n = cp*cn*(np.power(self.Q(ER)/math.sqrt(mp),4.)*self.FPhipn(ER)-np.power(vm*self.Q(ER),2.)*self.FS2pn(ER))
        g_n_p = cn*cp*(np.power(self.Q(ER)/math.sqrt(mp),4.)*self.FPhinp(ER)-np.power(vm*self.Q(ER),2.)*self.FS2np(ER))
        g_n_n = cn*cn*(np.power(self.Q(ER)/math.sqrt(mp),4.)*self.FPhinn(ER)-np.power(vm*self.Q(ER),2.)*self.FS2nn(ER))
        g = self.spin_dep(jx)*(g_p_p+g_p_n+g_n_p+g_n_n)/16  #units=ev^2

        return [g,h]


    def F1414(self,ER,cp,cn,vm): #jx
        """
    ...
    def F1515(self,ER,cp,cn): #jx vm
        ...
        g_p_p = cp*cp*(np.power(self.Q(ER)/(mp**(1/3)),6.)*self.FPhi2pp(ER)-np.power(math.sqrt(vm)*self.Q(ER),4.)*self.FS1pp(ER)/2)
        g_p_n = cp*cn*(np.power(self.Q(ER)/(mp**(1/3)),6.)*self.FPhi2pn(ER)-np.power(math.sqrt(vm)*self.Q(ER),4.)*self.FS1pn(ER)/2)
        g_n_p = cn*cp*(np.power(self.Q(ER)/(mp**(1/3)),6.)*self.FPhi2np(ER)-np.power(math.sqrt(vm)*self.Q(ER),4.)*self.FS1np(ER)/2)
        g_n_n = cn*cn*(np.power(self.Q(ER)/(mp**(1/3)),6.)*self.FPhi2nn(ER)-np.power(math.sqrt(vm)*self.Q(ER),4.)*self.FS1nn(ER)/2)
...
#double check 9,8 and the order of the S1 D form factor
    ...
    def F1215(self,ER,cp12,cn12,cp15,cn15,jx): #vm
        """
        ...
        g_p_p = cp*cp*(np.power(self.Q(ER)/math.sqrt(mp),4.)*self.FPhi2pp(ER)-np.power(vm*self.Q(ER),2.)*self.FS1pp(ER)/2)
        g_p_n = cp*cn*(np.power(self.Q(ER)/math.sqrt(mp),4.)*self.FPhi2pn(ER)-np.power(vm*self.Q(ER),2.)*self.FS1pn(ER)/2)
        g_n_p = cn*cp*(np.power(self.Q(ER)/math.sqrt(mp),4.)*self.FPhi2np(ER)-np.power(vm*self.Q(ER),2.)*self.FS1np(ER)/2)
        g_n_n = cn*cn*(np.power(self.Q(ER)/math.sqrt(mp),4.)*self.FPhi2nn(ER)-np.power(vm*self.Q(ER),2.)*self.FS1nn(ER)/2)
        g = -self.spin_dep(jx)*(g_p_p+g_p_n+g_n_p+g_n_n)/16  #units=ev^2
