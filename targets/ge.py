...
## Nuclear Functions For Ge





                                          ## Nuclear Functions For 70Ge
...
#F, JUN45, jj44b = symbols('F JUN45 jj44b')
...
# M channel
    ...
    def FMnp(self, JUN45, ER):
        y = self.Y(ER)
        return np.exp(-2.*y)*(1214.79 -3502.95 *y+3837.97 *pow(y,2.)-2030.76 *pow(y,3.)+554.24 *pow(y,4.)-75.7766 *pow(y,5.)+4.42145 *pow(y,6.)-0.0550292 *pow(y,7.)+0.000132836 *pow(y,8.))

    def FMnn(self, JUN45, ER):
        y = self.Y(ER)
...
# Sigma'' (Sigma2)
    ...
    def FS2pp(self, JUN45, ER):
        y = self.Y(ER)
        return 0
        
    def FS2pn(self, JUN45, ER):
        y = self.Y(ER)
        return 0

    def FS2np(self, JUN45, ER):
        y = self.Y(ER)
        return 0

    def FS2nn(self, JUN45, ER):
        y = self.Y(ER)
        return 0

# Sigma' (Sigma1)

    def FS1pp(self, JUN45, ER):
        y = self.Y(ER)
        return 0
        
    def FS1pn(self, JUN45, ER):
        y = self.Y(ER)
        return 0

    def FS1np(self, JUN45, ER):
        y = self.Y(ER)
        return 0

    def FS1nn(self, JUN45, ER):
        y = self.Y(ER)
        return 0

#Phi'' (Phi2)
    def FPhi2pp(self, JUN45, ER):
        y = self.Y(ER)
    ...
    def FPhi2np(self, JUN45, ER):
        y = self.Y(ER)
        return np.exp(-2.*y)*(39.9244 -71.4517 *y+46.6257 *pow(y,2.)-13.5302 *pow(y,3.)+1.70871 *pow(y,4.)-0.0725119 *pow(y,5.)+0.000531345 *pow(y,6.))

    def FPhi2nn(self, JUN45, ER):
        y = self.Y(ER)
        return np.exp(-2.*y)*(28.1754 -54.6766 *y+39.551 *pow(y,2.)-13.2472 *pow(y,3.)+2.09646 *pow(y,4.)-0.140826 *pow(y,5.)+0.00329371 *pow(y,6.))

#Phi' (Phi)
    def FPhipp(self, JUN45, ER):
        y = self.Y(ER)
        return 0
        
    def FPhipn(self, JUN45, ER):
        y = self.Y(ER)
        return 0

    def FPhinp(self, JUN45, ER):
        y = self.Y(ER)
        return 0

    def FPhinn(self, JUN45, ER):
        y = self.Y(ER)
        return 0

# Delta (D)
    def FDpp(self, JUN45, ER):
        y = self.Y(ER)
        return 0
        
    def FDpn(self, JUN45, ER):
        y = self.Y(ER)
        return 0

    def FDnp(self, JUN45, ER):
        y = self.Y(ER)
        return 0

    def FDnn(self, JUN45, ER):
        y = self.Y(ER)
        return 0


# M Phi2
    def FMPhi2pp(self, JUN45, ER):
        y = self.Y(ER)
...
# Sigma1 Delta
    def FS1Dpp(self, JUN45, ER):
        y = self.Y(ER)
        return 0
    
    def FS1Dpn(self, JUN45, ER):
        y = self.Y(ER)
        return 0

    def FS1Dnp(self, JUN45, ER):
        y = self.Y(ER)
        return 0

    def FS1Dnn(self, JUN45, ER):
        y = self.Y(ER)
        return 0   
...
# M channel
    ...
    def FMnp(self, jj44b, ER):
        y = self.Y(ER)
        return np.exp(-2.*y)*(1214.42 -3523.21 *y+3865.2 *pow(y,2.)-2036.23 *pow(y,3.)+551.766 *pow(y,4.)-75.2609 *pow(y,5.)+4.53602 *pow(y,6.)-0.0767866 *pow(y,7.)+0.000262912 *pow(y,8.))

    def FMnn(self, jj44b, ER):
        y = self.Y(ER)
...
# Sigma'' (Sigma2)
    ...
    def FS2pp(self, jj44b, ER):
        y = self.Y(ER)
        return 0
        
    def FS2pn(self, jj44b, ER):
        y = self.Y(ER)
        return 0

    def FS2np(self, jj44b, ER):
        y = self.Y(ER)
        return 0

    def FS2nn(self, jj44b, ER):
        y = self.Y(ER)
        return 0

# Sigma' (Sigma1)

    def FS1pp(self, jj44b, ER):
        y = self.Y(ER)
        return 0
        
    def FS1pn(self, jj44b, ER):
        y = self.Y(ER)
        return 0

    def FS1np(self, jj44b, ER):
        y = self.Y(ER)
        return 0

    def FS1nn(self, jj44b, ER):
        y = self.Y(ER)
        return 0

#Phi'' (Phi2)
    def FPhi2pp(self, jj44b, ER):
        y = self.Y(ER)
    ...
    def FPhi2pn(self, jj44b, ER):
        y = self.Y(ER)
        return np.exp(-2.*y)*(47.8393 -86.9937 *y+57.4149 *pow(y,2.)-16.8203 *pow(y,3.)+2.18029 *pow(y,4.)-0.102593 *pow(y,5.)+0.00105165 *pow(y,6.))

    def FPhi2np(self, jj44b, ER):
        y = self.Y(ER)
        return np.exp(-2.*y)*(47.8393 -86.9937 *y+57.4149 *pow(y,2.)-16.8203 *pow(y,3.)+2.18029 *pow(y,4.)-0.102593 *pow(y,5.)+0.00105165 *pow(y,6.))

    def FPhi2nn(self, jj44b, ER):
        y = self.Y(ER)
        return np.exp(-2.*y)*(53.9153 -106.721 *y+78.4707 *pow(y,2.)-26.6939 *pow(y,3.)+4.33836 *pow(y,4.)-0.30907 *pow(y,5.)+0.00782248 *pow(y,6.))

#Phi' (Phi)
    def FPhipp(self, jj44b, ER):
        y = self.Y(ER)
        return 0
        
    def FPhipn(self, jj44b, ER):
        y = self.Y(ER)
        return 0

    def FPhinp(self, jj44b, ER):
        y = self.Y(ER)
        return 0

    def FPhinn(self, jj44b, ER):
        y = self.Y(ER)
        return 0

# Delta (D)
    def FDpp(self, jj44b, ER):
        y = self.Y(ER)
        return 0
        
    def FDpn(self, jj44b, ER):
        y = self.Y(ER)
        return 0

    def FDnp(self, jj44b, ER):
        y = self.Y(ER)
        return 0

    def FDnn(self, jj44b, ER):
        y = self.Y(ER)
        return 0

# M Phi2
    def FMPhi2pp(self, jj44b, ER):
        y = self.Y(ER)
...
# Sigma1 Delta
    def FS1Dpp(self, jj44b, ER):
        y = self.Y(ER)
        return 0
    
    def FS1Dpn(self, jj44b, ER):
        y = self.Y(ER)
        return 0

    def FS1Dnp(self, jj44b, ER):
        y = self.Y(ER)
        return 0

    def FS1Dnn(self, jj44b, ER):
        y = self.Y(ER)
        return 0   
