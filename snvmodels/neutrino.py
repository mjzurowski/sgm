...
#            result = 
            ...
            result=f17NDist.dist(Ev/MeV)/MeV
        elif source == "solar_o15":
            o15NDist = SolarNDist("o15",0.05031e10)
            result=o15NDist.dist(Ev/MeV)/MeV
        elif source == "solar_n13":
            n13NDist = SolarNDist("n13",0.05712e10)
            result=n13NDist.dist(Ev/MeV)/MeV
        elif source == "solar_hep":
            hepNDist = SolarNDist("hep",0.000000788e10)
            result=hepNDist.dist(Ev/MeV)/MeV
        elif source == "solar_b8":
            b8NDist = SolarNDist("b8",0.0005822e10)
            result=b8NDist.dist(Ev/MeV)/MeV
        elif source == "solar_pp":
            ppNDist = SolarNDist("pp",5.938e10)
            result=ppNDist.dist(Ev/MeV)/MeV
        elif source == "solar_be7":
            result = sympy.DiracDelta(x-384.3e3)* 0.103*0.4857e10 + sympy.DiracDelta(x-861.8e3)* 0.897*0.4857e10/MeV
        elif source =='solar_pep':
            result = sympy.DiracDelta(x-1442e3)*0.01401e10/MeV
        else:
    ...
    def dRdEd(self,Ev,Ed,E_ave,channel,source):
        ...
        if channel == "IBD-naive+": 
            ...
            def Sig(Ev, Eout, channel,source):
                if source == 'supernova_e' or source == 'supernova_ea' or source.split('_')[0]=='solar':
                    epsilonN=-0.5-sin2thetaW
                    epsilonP=-sin2thetaW
                else:
                    epsilonN=0.5-sin2thetaW
                    epsilonP=-sin2thetaW
                if source == 'supernova_e' or source == 'supernova_mu' or source == 'supernova_tau' or source.split('_')[0]=='solar':
            ...
            integral = integrate.quad(lambda Evl:integrand(Evl,Ed,E_ave,channel,source),Emin,max(Ev))[0]
